          ...
          <div style={{ padding: '8px 0' }}>
            {gainers.slice(0, 10).map((p, i) => (
              <div key={p.symbol} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px' }}>
                <div style={{ fontSize: 11, color: '#252525', fontWeight: 600, width: 16, textAlign: 'right' }}>{i + 1}</div>
            ...
            <tbody>
              {data.byYear.map(y => (
                ...
                >
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right', fontFamily: "'DM Mono', monospace", fontSize: 13, color: '#c0c0c0' }}>{fmtCurrency(y.divs)}</td>
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right', color: '#888888', fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{fmtCurrency(y.deposits)}</td>
                  <td style={{ padding: '10px 14px', textAlign: 'right', color: '#888888', fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{fmtK(y.buyVol)}</td>
                  <td style={{ padding: '10px 14px', textAlign: 'right', color: '#888888', fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{fmtK(y.sellVol)}</td>
                </tr>
              ))}
              <tr style={{ borderTop: '2px solid #252525', background: '#080808' }}>
                <td style={{ padding: '10px 14px', fontWeight: 700, color: '#ffffff' }}>All Time</td>
                ...
                <td style={{ padding: '10px 14px', textAlign: 'right', fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, color: '#c0c0c0' }}>{fmtCurrency(data.allTime.dividends)}</td>
                <td style={{ padding: '10px 14px', textAlign: 'right' }}>
                ...
                <td style={{ padding: '10px 14px', textAlign: 'right', color: '#c0c0c0', fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmtCurrency(data.allTime.deposits)}</td>
                <td style={{ padding: '10px 14px', textAlign: 'right', color: '#888888', fontFamily: "'DM Mono', monospace" }}>{fmtK(data.allTime.buyVol)}</td>
                <td style={{ padding: '10px 14px', textAlign: 'right', color: '#888888', fontFamily: "'DM Mono', monospace" }}>{fmtK(data.allTime.sellVol)}</td>
              </tr>
            </tbody>
          </table>
        </div>
